  ...
  class FetchQuery
    ...
    def call
      ...
      users = User.all.select(
        ...
        "mobile_number", "is_admin", "created_at", "updated_at", "user_type", "company_name"
      ).paginate(page: page).order('created_at DESC')
    else
      users = User.where(
      "first_name like ? or last_name like ? or email like ? or forward_email like ? or mobile_number like ? or company_name like ?",
       "%" + search_term + "%","%" + search_term + "%","%" + search_term + "%","%" + search_term + "%","%" + search_term + "%", "%" + search_term + "%" ).select(
        "id", "first_name", "last_name", "email", "forward_email", "status",
        "mobile_number", "is_admin", "created_at", "updated_at","user_type", "company_name"
      ).paginate(page: page).order('created_at DESC')
    end
